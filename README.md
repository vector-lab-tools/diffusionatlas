...
**Version:** 0.4.0
**Date:** 9 August 2026
**Licence:** GNU General Public License, version 3 or later (GPL-3.0-or-later). See `LICENSE` and `COPYING`. All prior versions are retroactively relicensed under the same terms.
...
Diffusion Atlas's local backend is **designed for single-tenant localhost-only operation**. The protections are layered:

1. **Loopback bind (`127.0.0.1`).** The kernel rejects TCP connections from anywhere outside the same machine before any HTTP code runs. This is the actual security boundary — everything else is convenience or defence-in-depth.
2. **CORS regex** restricting browser origins to `http(s)://(localhost|127.0.0.1):<any-port>`. Catches cross-origin browser pages; doesn't help against same-machine attackers.
3. **`X-Diffusion-Atlas` identity handshake.** Every state-touching endpoint requires this header (`GET /health` is exempt for liveness polling). The header is a **self-identification gesture, not authentication** — its value (`"1"`) is hardcoded in the source, which is public on GitHub. What it prevents in practice: accidental cross-app collisions on a busy localhost (a random web app you forgot was running won't blunder into `/generate`), and the casual-CSRF probe class where a malicious page targets diffusion backends generically. A targeted attacker who reads this README obviously won't be slowed down by it. The friendly 401 message also acts as a discoverability gate for researchers who curl the backend directly.

**Do not expose this backend to the public internet on this design.** The reason isn't that the header value is now public (it always was — sniff one network request and you have it); it's that **public deploy removes the loopback boundary**, which is the only meaningful protection. If you want a remote-deploy path (Modal, fal Custom Apps, Replicate Cog, RunPod Serverless — see the Roadmap), replace the identity handshake with a per-deploy bearer token configured via the Settings panel, keep the binding behind HTTPS, and treat the backend as a real authenticated service.
...
- [x] **Sweep + Neighbourhood get the contact-sheet aesthetic** (dark sprocketed strip + white edge-print metadata band per lane, matching the Trajectory FilmStrip). **Generic stacking primitive** extracted to `useLayerStack` + `<LayerStackPanel>` — Trajectory refactored onto it; Sweep / Neighbourhood / Bench can adopt the same shape in ~20 lines each. **Persistent layers** in IDB (`op_state` store via structured clone — Float32Array latents survive). **Per-operation Reset button** wired through the shared panel. **CFG plain-English captions** per cell (`1 — no amplification`, `7.5 — balanced default`, `12 — aggressive`, etc.) via shared `cfgCaption()`. **CFG dropdown** replaces the free-form input in Trajectory / Neighbourhood / Bench (Sweep keeps the multi-value list). **Default CFG list** dropped fragile `2.5` slot (sat in the empirical fp32-attention-NaN range on MPS); now `1, 4, 7.5, 12, 18`. **Drift curve moved to Deep Dive** with proper gridlines + axis labels + a plain-English caption; **drawn natively in the PDF** via `jsPDF` line primitives (no SVG-to-PNG roundtrip). **PNG export** for the contact-sheet film reel sits next to the SVG button (universal compatibility). **Step Inspector** now a click-to-open modal with a 200×200 thumbnail, full scalar dl, RGB histogram of the decoded preview, and **Play / Stop + speed selector** so you can watch the denoising as a looping animation. **EulerDiscreteScheduler** pinned (DPM++ 2M produced sporadic NaN on MPS at certain CFGs); scheduler is **reset to a fresh instance on every forward** to dodge the `step_index` leak that produced `IndexError: index 21 out of bounds`. **bfloat16 attempt for SD 1.5 reverted** because bf16's 7-bit mantissa quantises `alphas_cumprod` enough to break some scheduler index calcs on MPS. **Cell error UX**: long backend diagnostics shortened to `model couldn't render` / `out of memory` / `resolution too large` etc. with the verbatim message in the hover tooltip, separated by a divider. **NaN tooltip** explains IEEE-754 NaN concretely. **Help dropdown** gains "What is MPS warmup?" entry (v0.3.13)
- [x] **Step Inspector ping-pong playback** (default — bounces between noise and final, preserves trajectory symmetry); ↔ / ↻ toggle for the classic forward-loop. **UMAP projection bug fixed** — saved-layer points cached PCA at save time and the toggle only reprojected the live in-flight latents; now a single shared `project()` callback reprojects every layer on toggle, and new layers capture with the current projection mode. **Vector Lab canonical branding refreshed** from upstream Pages repo: navy `#0a1f44` VL monogram replaces the gold wavy-grid mark (Header / StatusBar / AboutModal); tool icon red shifted from `#8b3a3a` deep burgundy to `#b97070` dusty rose. **CORS loosened** from a hardcoded `:3000` allowlist to a regex (`http(s)://(localhost|127.0.0.1):<any-port>`) so sibling Vector Lab apps occupying `:3000` no longer break Diffusion Atlas on `:3001`. **`X-Diffusion-Atlas` identity handshake** added — every state-touching endpoint (`/warmup`, `/trajectory`, `/score`, `/generate`) requires the header; `/health` is exempt for liveness polling. Header value is hardcoded `"1"`; the design intent is self-identification (prevents accidental cross-app collisions on a busy localhost), not authentication (the value is in the source). See the **Security Model** section. Friendly 401 message names the header so researchers scripting the backend directly know exactly what to add (v0.3.14)
...
- [ ] **Remote-backend deploy path** (long-range): Atlas operations need a your-own-process diffusers runtime, currently localhost-only. The same `backend/` FastAPI app could be wrapped as a Modal / fal Custom App / Replicate Cog / RunPod Serverless container so researchers without a CUDA box can run Trajectory + sweeps at SDXL / SD3 / FLUX scale on rented GPU. The frontend already accepts a configurable `localBaseUrl` — generalising it to "Backend URL" + bearer token in Settings is a small change. **Modal** is the favoured target (NDJSON streaming maps directly onto async generators; warm-pool keeps the pipeline hot across a research session; scales to zero when idle; $30/month free tier covers most academic work). The Help / About panel would then describe three deploy modes: laptop, rented serverless GPU, or self-hosted server
