...
export const GLOSSARY: Record<string, string> = {
  ...
  Steps: "Number of denoising steps. More steps usually means cleaner output but slower generation. With the EulerDiscreteScheduler the backend pins (Euler is the MPS-stable choice — DPM++ produces NaN at certain CFGs on MPS), suggested tiers for SD 1.5 are: 12 = low (quick smoke test, subject roughly there, detail soft), 20 = normal (default — coherent images, fast-enough feedback), 30 = high (crisper detail, longer trajectories with more curvature to scrutinise in the Deep Dive), 50+ = research-grade (marginal gains for stress-testing CFG or fine text). Distilled models like FLUX Schnell or LCM go the other way and want 1–4 steps.",
  CFG: "Classifier-Free Guidance scale — how strongly the model amplifies the prompt's pull on the denoising trajectory. At each step the U-Net runs twice (once with the prompt, once with an empty prompt) and the result is unconditional + CFG × (conditional − unconditional). So CFG 0 = pure unconditional, prompt fully ignored; CFG 1 = prompt-conditioned output as-is, no amplification (the prompt is still in the loop, just not pushed harder); CFG 4-5 = soft amplification; CFG 7.5 = the conventional balanced default; CFG 12-15 = aggressive amplification with rising oversaturation; CFG 20+ = mode collapse, burned colours. The Guidance Sweep operation traces this lever.",
