...
export const GLOSSARY: Record<string, string> = {
  ...
  CFG: "Classifier-Free Guidance scale — how strongly the model pushes the denoising trajectory toward the prompt. At each step the U-Net runs twice (once unconditional, once on the prompt) and the difference is amplified by CFG. 1.0 = ignore the prompt entirely (pure unconditional generation); 4-5 = soft / atmospheric; 7.5 = the conventional balanced default; 12-15 = aggressive prompt adherence with rising oversaturation; 20+ = mode collapse, burned colours, no surprise. CFG is the lever the Guidance Sweep operation traces.",
